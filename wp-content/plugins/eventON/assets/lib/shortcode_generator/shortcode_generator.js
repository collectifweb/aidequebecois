...
jQuery(document).ready(function($){
	...
	// trigger actions		
		...
		$('body').on('evo_block_control_clicked',function(event, sc, type, other_id){
			//console.log(sc);
			//console.log(other_id);

			$(other_id.target).addClass('YY');
		});
		...
		// when a sc field is removed 
		BODY.on('evo_block_sc_trigger_change',function(event, sc_key, sc_value, type, textarea, bid){
			all_sc = $(textarea).val();

			var scdata = $('this').evo_process_sc( all_sc );

			var new_sc = scdata.base;
			if( scdata.vars){
				var new_vars = {};
				$.each( scdata.vars, function(f,v){
					if( v == '') return;
					if( f === undefined || f == 'undefined') return;
					
					// remove or change for matching key
					if( f == sc_key ){
			...
			scdata['vars'] = new_vars;

			scdata['type'] = 'block';
			scdata['other_id'] = bid;

			BODY.trigger('evo_block_sc_changed',[ new_sc_ , scdata]);

			//$(textarea).val( new_sc_ ).change();
	...
	// functionalities s4.6
		$.fn.evo_process_sc = function( sc){
			sc = sc.replace('[','');
			sc = sc.replace(']','');
		...
		$.fn.evo_shortcode_init = function(opt){
			...
			var interaction = function(){
				...
				$(window).on('resize',function(){
					if( generator_in.hasClass('moved')){
						generator_in.animate({scrollLeft: generator.width()}, 2);
					}
				});

				// search
				generator.on('keyup','.evo_SG_search_in',function(event ){
					var keycode = (event.keyCode ? event.keyCode : event.which);
					var typed_val = $(this).val().toLowerCase();

					PAR = $(this).closest('.evo_shortcode_generator_box').find('.step2_in');

					PAR.find('.fieldline').each(function(){
						if( $(this).find('label').length == 0 ) return;
						if( $(this).find('label').html() === undefined ) return;
						const n = $(this).find('label').html().toLowerCase();
						
						if( typed_val == ''){
							$(this).show(); PAR.removeClass('srON');
						}else{
							PAR.addClass('srON');
				...
				generator.on('click','#ajdePOSH_back',function(){
					...
					generator.find('h3.notifications').removeClass('back srON');	
					generator_in.animate({scrollLeft: 0}, 200).removeClass('moved');			
					generator_in.find('.step2_in').fadeOut();
				...
				generator.on('click', '.trig_afterst .ajde_yn_btn',function(){
					O = $(this);
					if( O.hasClass('NO')){
						O.parent().next('.ajde_afterst').show();
					}else{
						O.parent().next('.ajde_afterst').hide();
					}
				});
				...
				// copy shortcode
				generator.on('click','.evoSC_copy_trig', function(){
					const COP_VAL = $(this).siblings('p').html();
				    ...
				    $(this).evo_elm_show_tooltip('Shortcode Copied', 2000);
				...
				generator.on('click','.ajdePOSH_insert',function(){
					var obj = $(this);
					var shortcode = generator.find('#ajdePOSH_code').html();	
			...
			var goto_inside_step = function(force){
				...
				if( Object.keys( data.vars ).length > 0 || force){
					...
					if( step.length>0){
						...
						//generator_in.animate({'margin-left':'-'+ sg_width + 'px'},'fast');
						generator_in.animate({scrollLeft: distance}, 200).addClass('moved');
						

						//search 
						__sr = step.data('sr') ? 'srON':'';
						
						// show back button
						generator.find('#ajdePOSH_back').animate({'left':'0px'},'fast');		
						generator.find('h3.notifications').addClass('back '+ __sr);
