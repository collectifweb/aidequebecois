...
// header save changes button
	...
	// after settings are saved @4.8.1
	.on('evo_ajax_success_evo_save_lang_settings',function(event, OO, data , el){

		// Check if the message div already exists
	    if ($('.evo_updated.updated.fade').length === 0) {
	        var messageDiv = $("<div class='evo_updated updated fade'><p>Settings Saved</p></div>").insertBefore(el);
...
// dynamic checkbox value passing
	$('body').on('click','.evo_settings_checkbox',function(){
