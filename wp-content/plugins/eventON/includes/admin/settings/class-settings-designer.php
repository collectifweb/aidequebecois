		...
		foreach($cutomization_pg_array as $cpa=>$cpav){								
			...
			if( $tab_type !='empty'){ // to not show the right side
				...
				foreach($cpav['fields'] as $field){
					...
					// new label for addons @4.6.4
					if( isset($field['ver']) && isset($field['compare_ver']) && $field['ver'] == $field['compare_ver']){
						$legend_code .= "<span class='new evonewtag' title='".__('New in version','eventon') .' '. $field['compare_ver']."'>new</span>";
					}
					...
					switch ($field['type']){
						...
						case 'notice':
							$rightside .= EVO()->elements->get_element(array(
								'type'=>'notice','name'=> $field['name'],
								'row_class'=>'ajdes_notice',
								'tooltip'=> isset( $field['legend'] ) ? $field['legend']:'',
								'tooltip_position'=>'L'
							));
						break;
						//IMAGE
						...
						case 'checkboxes':
							...
							$meta_arr= (!empty($data[ $field['id'] ]) )? $data[ $field['id'] ]: null;
							$default_arr= (!empty($field['default'] ) )? $field['default']: null;

							ob_start();
							
							echo "<p class='evo_settings_checkbox acus_line acus_checks'><span style='padding-bottom:10px;'>".__($field['name'],$textdomain)."</span>";
						...
						case 'yesno':						
							...
							$__default = (!empty( $field['default'] ) && $data[$field['id'] ]!='yes' )? 
								$field['default']
								:$yesno_value;
