	...
	public function eventon_admin_init() {	
		...
		// EVENT POSTS
			if ( $postType && $postType == "ajde_events" ) {	
				...
				// print on page styles and scripts
				add_action('admin_head', array($this, 'eventedit_inline_styles'));
		...
		// event edit page content
			include_once(  EVO_ABSPATH.'/includes/admin/post_types/class-meta_boxes.php' );
		...
		// eventon settings only 
			...
			if($pagenow =='admin.php' && isset($_GET['page']) && ($_GET['page']=='eventon' || $_GET['page']=='action_user')
				...
				do_action('eventon_admin_settings_pageonly_init');
			}
...
// admin menus
	...
	/** Include and display the settings page. */
		function eventon_settings_page() {
			...
			include_once(  EVO_ABSPATH.'/includes/admin/settings/class-settings-appearance.php' );
			include_once(  EVO_ABSPATH.'/includes/admin/settings/class-settings-scripts.php' );
...
// admin styles and scripts
	// EVENTON Posts pages
		...
		function eventon_admin_post_script() {
			...
			if ( $typenow == '' || $typenow == "ajde_events" ) {
				...
				//wp_register_script( 'evo_charts',EVO()->assets_path.'js/lib/chart.js',array('jquery'), EVO()->version, true);
				wp_register_script( 'evo_charts','https://cdn.jsdelivr.net/npm/chart.js',array('jquery'), EVO()->version, true);
				...
				wp_enqueue_script(
					'evcal_backend_post',
					AJDE_EVCAL_URL.'/assets/js/admin/event-post.js', 
					array('jquery','jquery-form','jquery-ui-core','jquery-ui-datepicker', 'wp-block-editor', 'wp-element', 'wp-editor'), 
					EVO()->version, 
					true );
				wp_enqueue_script("jquery-ui-core");
				wp_enqueue_style('wp-editor');

				// Localize the script to pass initial data
		        wp_localize_script('evcal_backend_post', 'blockEditorData', 
		        	['content' => get_post_meta($post->ID, '_custom_paragraph_content', true),]
		        );
	...
	// scripts and styles for wp-admin 4.6.8
		function wp_admin_scripts_styles(){
			...
			if( (!empty($pagenow) && $pagenow=='admin.php')
			 && (isset($_GET['page']) && ($_GET['page']=='eventon'|| $_GET['page']=='action_user'|| $_GET['page']=='evo-sync') ) 
				...
				// LOAD custom google fonts for skins		
					$gfont = ( is_ssl() ? "https" :"http" ). "://fonts.googleapis.com/css?family=Open+Sans:300italic,400,300";
			...
			//wp_enqueue_style( 'evo_fonta',AJDE_EVCAL_URL.'/assets/fonts/all.js',array(), EVO()->version);
	...
	// event edit only inline styles
	// @since 4.2.3
		function eventedit_inline_styles(){
			echo "<style id='evo_event_edit_styles'>". apply_filters('evo_eventedit_inline_styles','')."</style>";
		}
...
// Shortcode on Editor
	...
	//This callback adds eventon shortcode generator button to the toolbar
	...
	// shortcode generator
		function eventon_shortcode_button($context) {	
			...
			//our popup's title
		  	...
		  	//append the icon
		  	$context .= "<a id='evo_shortcode_btn' class='evo_admin_btn btn_prime evolb_trigger_shortcodegenerator' title='{$title}' href='#'>{$text}</a>";
...
// Supporting functions
	...
	// print event meta box loading bars html content
	// @since 4.2.3 => 4.8.2 moving to EVO_Elements_Admin
	function print_metabox_loading_html(){
		?>
