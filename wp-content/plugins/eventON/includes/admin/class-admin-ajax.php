...
class EVO_admin_ajax{
	...
	// @update 4.7.4
		...
		private function get_event_csv_fields( $et_count, $cmd_count){
			...
			$csv_fields = array(
				...
				'evcal_location'	=>'location_address',	// address		
		...
		public function settings_save(){
    		...
    		// all other settings
					...
					foreach($form_data as $settings_field => $settings_value ){
						...
						// skip fields
						if(in_array($settings_field, array( 'option_page', 'action','_wpnonce','_wp_http_referer','evcal_noncename',
						))){	continue;	}
						...
						// sanitize the fields
							...
							$new_settings[ $settings_field ] = 
								!is_array($settings_value) ? sanitize_text_field( $settings_value ) : 
								$settings_value;
						}	
					...
					// save settings
						...
						$new_settings = apply_filters('evo_save_settings_optionvals', $new_settings, $current_tab);

						EVO()->cal->set_option_values( $new_settings );

					// save custom styles and php code
						if( isset($new_settings['evcal_styles']) ) update_option('evcal_styles', strip_tags(stripslashes($new_settings['evcal_styles'])) );

						if( isset($new_settings['evcal_php']) )	update_option('evcal_php', strip_tags(stripslashes($new_settings['evcal_php'])) );
	...
	// Diagnose
		...
		function admin_test_email(){
			...
			// verify nonce
				if( empty( $_REQUEST['nonce'] ) || !wp_verify_nonce( wp_unslash( $_REQUEST['nonce'] ), 'eventon_admin_nonce')) {
					wp_die('Security Check Failed!');
				}
