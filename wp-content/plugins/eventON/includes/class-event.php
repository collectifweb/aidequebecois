	...
	// DATE TIME
		...
		function get_start_end_times($custom_ri='', $return_type = 'both', $utc = false){
			...
			if($this->is_repeating_event() && !empty( $custom_ri ) ){
				...
				// set RI for this event instance
			...
			//echo $this->start_unix.'/'. $this->start_unix_raw .'--';
	...
	// @updated 4.5.5
		...
		function get_ics_content($include_repeats = false){
			...
			// start and end time
				...
				$dDATA = $this->get_time_in_GMT();

				$format =  $this->is_all_day() ? 'Ymd' : 'Ymd\THi';

				$this->DD->setTimestamp( $dDATA['start'] );
				$start = $this->DD->format( $format );

				$modified_end_unix = $this->is_all_day() ? $dDATA['end'] + 86400: $dDATA['end'];
				$this->DD->setTimestamp( $modified_end_unix );
				$end = $this->DD->format( $format );		
