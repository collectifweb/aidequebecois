		...
		switch($type){
			...
			case 'images':
				...
				<p class='evo_metafield_image multi'>
					...
					for($x = 1; $x<= $max_images ; $x++){
						...
						echo "<span class='evo_img evolm_img_select_trig evo_img_{$x} ". ( !empty($img_url) ? 'on':'' ) ."' style='background-image:url(". $img_url .");' data-index='{$x}' >
			...
			case 'human_validation':
				...
				echo "<p class='evo_field_container'>";
					...
					echo "<input class='{$field_class} evoelm_hval_val' type='{$field_type}' name='{$id}' {$placeholder} data-val='89r329{$value}8932ufe'/>";
			...
			case 'select':		
				...
				if( !empty($name)) echo "<label>$name $legend_code</label>"; 
			...
			case 'dynamic_select':
				...
				echo "<div class='evo_elm_row evo_elm_dynamic_select {$id} {$row_class} evoposr' style='{$row_style}'>";

					if( !empty($name)) echo "<label>$name $legend_code</label>"; 


					if( empty($default)) $default = reset($options);

					echo "<button class='evo_elm_dynamic_select_trig evocurp '><div class='evoelm_ds_in evodfx evofx_dr_r evogap10 evofx_ai_c'>";
						echo "<div class='evoelm_ds_current evofz14 evoff_2'>" .$default.  "</div>";
	...
	function print_date_time_selector($A){
		...
		if($selector != 'time' ):
			echo " <span class='evo_date_edit'>
				<input id='evo_{$type}_date_{$rand}' class='". ($disable_date_editing?'':"datepicker{$type}date")." ". ($assoc != 'rp'? 'req':'')." {$type} evo_dpicker {$subtype}' readonly='true' type='text' data-role='none' name='event_{$type}_date' value='".$date_val."' data-assoc='{$assoc}' />	
...
// General Settings Element support - 4.6 @updated 4.7.2
	...
	function _print_settings_toggle_nester_start( $data){
		...
		?>
		<div class='evopad20 evo_borderb <?php echo $toggle_class;?>'>			
			<div class=''>
				<?php 
