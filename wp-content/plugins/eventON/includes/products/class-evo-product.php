	...
	// WP CONNECT
		// product information for details lightbox
			public function plugin_details($false, $action, $response){
				...
				$response->rating =  	(4.37*20);			
				$response->num_ratings =  	2373;
				$response->active_installs =  	($this->get_prop('active_installs') ? $this->get_prop('active_installs'): 63800);
				$response->requires_php =  	($this->get_prop('requires_php') ? $this->get_prop('requires_php'):'8.0 or higher');
