...
// DATE & TIME
	...
	// pretty time on event card - after 4.6 deprecating
	...
	// RETURN: formatted event time in multiple formats @4.6
	// $tz = cal, utc, or event timezone
	function eventon_get_formatted_time($row_unix, $tz = 'cal', $dt_format = '' ){
		/*
				D = Mon - Sun
			1	j = 1-31
				l = Sunday - Saturday
		...
		*/
		...
		$DD->setTimestamp( (int)$row_unix);
				
		$key = array('D','j','l','N','S','n','F','t','z','Y','g','i','a','M','m','d','H', 'A', 'y','G');

		/*
		// if passed datetime format array
		if( !empty($dt_format) ){
			$key = is_array( $dt_format ) ? $dt_format : str_split( $dt_format ); 
		}
		*/

		$date = explode('-',$DD->format( implode('-', $key) ));
	...
	*/
	function eventon_get_timeNdate_format($evcal_opt='', $force_wp_format = false){
		...
		if( (!empty($evcal_opt) && isset($evcal_opt['evo_usewpdateformat']) && $evcal_opt['evo_usewpdateformat']=='yes') || $force_wp_format){
	...
	*/
	function eventon_return_timely_names_($type, $data, $len='full', $lang=''){
		...
		if($type=='day'){
			...
			if($len=='full'){			
				...
				$_not_value = isset($eventon_day_names[ $text_num] ) ?
					ucfirst( $eventon_day_names[ $text_num] ) : $text_num;					
		...
		// day number to name
			...
			if($len=='full'){	
				...
				$_not_value = !empty($eventon_month_names[ $text_num])?
					ucfirst( $eventon_day_names[ $text_num] ):'';
		...
		// month number to name
			...
			if($len == 'full'){
				$option_name_prefix = 'evcal_lang_';
				$_not_value = !empty($eventon_month_names[ $text_num])? 
					ucfirst( $eventon_month_names[ $text_num] ):'';
	...
	// Updated: 4.6
		function eventon_get_repeat_intervals($unix_S, $unix_E, $post_data = ''){

			$help = new evo_helper();
			$postdata = empty( $post_data ) ? $help->sanitize_array($_POST) : $post_data;
			...
			// @4.6.9
			$proceed = apply_filters('evo_event_generate_repeat_data', true, $postdata , $DD );
			if( !$proceed ) return $repeat_intervals;
					...
					foreach($_post_repeat_intervals as $field => $interval){
						...
						if(isset($interval['type']) && isset($interval['type'])=='dates'){
							...
							if (empty($interval[0]) || empty($interval[1])) {
							    error_log('Missing start or end date in interval: ' . print_r($interval, true));
							...
							// override with passed on date time format
							if( isset($interval['df'])) $def_date_format = esc_html( $interval['df'] );
							if( isset($interval['tf'])) $time_format = esc_html( $interval['tf'] );

							$timezone = EVO()->calendar->timezone0 ?: new DateTimeZone('UTC');

							$datetime_start = DateTime::createFromFormat($def_date_format.' '. $time_format, $interval[0], $timezone );
							$datetime_end = DateTime::createFromFormat($def_date_format.' '. $time_format, $interval[1], $timezone );
							
							if ($datetime_start && $datetime_end) {
						        $repeat_intervals[] = array(
						            $datetime_start->format('U'),
						            $datetime_end->format('U')
						        );
						        $count .= $field . ' ';
						    } else {
						        error_log('Failed to parse interval: ' . print_r($interval, true));
						        continue;
						    }
...
// TEMPLATE
	...
	// @updated 4.6
	function eventon_get_template_part( $slug, $name = '' , $preurl='', $return_template = false ) {
		$cache_key = sanitize_key( implode( '-', array( 'template-part', $slug, $name, EVO()->version ) ) );
		$template  = (string) wp_cache_get( $cache_key, 'eventon' );

		// if pre url is provided append that to file
		$file_name = ( !empty( $preurl ) ? $preurl :'') ."/{$slug}-{$name}.php";
		

		// if no cache templates
		if( !$template){
			if ( $name ) {

				$locations = apply_filters('evo_template_locations', 
					array(
						"{$slug}-{$name}.php",
		...
		if ( $template ) {
			if( $return_template ) return $template;
...
// GENERAL
	...
	function evo_get_event_template($opt){
		...
		return 'archive-ajde_events.php';
...
// SUPPORT FUNCTIONS
	...
	// Generate location latLon from address @4.7.2
