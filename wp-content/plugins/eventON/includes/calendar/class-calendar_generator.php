...
class EVO_generator extends EVO_Cal_Time{
	...
	// time date values
		public $GMT, $DD, $timezone, $timezone0, $current_time, $time_format, $date_format, $utc_time, $current_time0, $cal_tz_string,  $cal_utc_offset, $cal_tz, $cal_tz_gmt, $SC;
		private $utc_DD;
	...
	/**	Construction function	 */
		public function __construct(){
			...
			require_once('class-calendar-event-schema.php');
	...
	// INIT		
		function init(){			
			...
			$this->shell = new evo_cal_shell($this);
	...
	// PARSE and process SHORTCODE arguments
		function process_arguments($args='', $set_date_range = true){
			...
			// tiles shortcode altering
				if(isset($args['tiles']) && $args['tiles']=='yes' && isset($args['ux_val']) && $args['ux_val'] == '1'){
					$args['ux_val'] = 3;
				}
			...
			$this->SC = new EVO_Cal_Shortcode( $this->shortcode_args );

			$this->shortcode_args = $this->SC->process();
			...
			if( $number_of_months > 1 && $search_all == 'no'){
				...
				if($proceed){		
					...
					for($x=0; $x< $number_of_months ; $x++){
						...
						if( $sep_month == 'yes'){
							$content.= "<div class='evcal_month_line' data-d='eml_{$_m}_{$_y}' data-su='{$SU}'><p>".eventon_returnmonth_name_by_num($_m). ($show_year == 'yes'? ' '.$_y:'') ."</p></div>";
					...
					if($EVENT->is_repeating_event()){
						...
						if(!empty($repeat_intervals) && is_array($repeat_intervals)){
							...
							foreach($repeat_intervals as $index => $interval){
								...
								// if hide cancels @4.6.3
								if( $hide_cancels == 'yes' && $EVENT->is_cancelled() ) continue;
								...
								// pluggable event processing
									if( apply_filters('evo_events_cycle_filter_event_proceed', true, $EVENT, $SC ) == false ) continue;
	...
	/** GENERATE individual event data	for event list array */
		public function generate_event_data(
			...
			// Initial variables
				...
				$__is_tiles_on = !empty($SC['tiles']) && $SC['tiles']=='yes' ? true: false;
			...
			foreach($event_list_array as $event_):
				...
				// set how a single event would interact
					...
					// Calendar UX overrides
						...
						$event_ux_val = ($calendar_ux_val !='0')? 
							$calendar_ux_val:
							( ($__is_tiles_on && $event_ux_val == 1)? 3:	$event_ux_val );
				...
				// EACH DATA FIELD
					...
					// EVENT DESCRIPTION						
						if(!empty($EVENT->content) ){
							$event_full_description = $EVENT->content;
						}else{
				...
				// BUILD EVENT CARD					
					...
					// if UX to be open in new window then use link to single event or that link	
						$_rest_href = '';			
						if($EVENT->get_prop('evcal_exlink') ){
							if( $event_ux_val =='4' || $event_ux_val =='2' )
								$_rest_href = 'href="'.$event_permalink .'"';
						}

						// external link is set and NOT slide down
				...
				// CLASES - attribute
					...
					// pass if eventtop text color need white or black based on event color @added 4.8
					$_eventClasses [] = ( eventon_is_hex_dark( $event_color )) ? 'clrW':'clrD';	
		...
		public function get_all_event_data($args = array()){
			...
			if($events->have_posts()):
				while($events->have_posts()): $events->the_post();
					...
					// repeating event
						if( $EVENT->is_repeating_event() ){
							$output[$event_id]['repeats'] = $EVENT->get_repeats_adjusted();
							$output[$event_id]['repeat_type'] = $EVENT->get_prop('evcal_rep_freq');
							$output[$event_id]['repeat_gap'] = $EVENT->get_prop('evcal_rep_gap');
							$output[$event_id]['repeat_count'] = $EVENT->get_prop('evcal_rep_num');
							$output[$event_id]['repeat_mode'] = $EVENT->get_prop('evp_repeat_rb_wk'); // single day days of week
						}

					// Event timezone
						$output[$event_id]['event_timezone'] = $EVENT->get_timezone_key();
					...
					// time ext type
						...
						$output[$event_id]['time_ext_type'] = $EVENT->get_time_ext_type();
					...
					// Organizer
						...
						if ( $organizer_terms && ! is_wp_error( $organizer_terms ) ){
							...
							// organizer initial
								...
								$output[$event_id]['organizer_name'] = $organizer_terms[0]->name;
							...
							// organizer email
							if(!empty( $orgTermMeta['evcal_org_contact_e']))
								$output[$event_id]['organizer_email'] = stripslashes($orgTermMeta['evcal_org_contact_e']);
							
							// organizer phone
							if(!empty( $orgTermMeta['evcal_org_contact_phone']))
								$output[$event_id]['organizer_phone'] = stripslashes($orgTermMeta['evcal_org_contact_phone']);
	 ...
	 */
		...
		function get_supported_shortcode_atts(){
			$SC = new EVO_Cal_Shortcode(array());
			return $SC->defaults();
		}
