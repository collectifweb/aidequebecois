...
// main image
	if( $object->main_image && is_array($object->main_image)):
		...
		echo "<div class='evocard_main_image evo_img_triglb evobr15 evobgsc evobgpc evodfx evofx_jc_c evofx_ai_c evofz48 {$img_sty}' style='height:{$__height}px; background-image:url({$BGURL});' ". $this->helper->array_to_html_data( $_mi_data ) ." data-t='". evo_lang('Loading Image') ."..'>{$IMG}</div>";
		echo "</div>";
