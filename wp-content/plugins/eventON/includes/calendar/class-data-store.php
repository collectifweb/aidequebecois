...
class EVO_Data_Store{
	...
	public $post_author, $author, $title, $post_title, $post_name, $post_date, $raw_content, $post_content, $content, $excerpt, $post_excerpt, $post_status, $post_parent, $time, $post_password;
...
// meta data
	...
	public function save_meta($key, $value){
		...
		// update global object instance of event 4.6.5
		if(array_key_exists('EVO_props', $GLOBALS) ){
			global $EVO_props;
			if(isset($EVO_props[$this->ID])){
				$GLOBALS['EVO_props'][$this->ID][$key] = $value;
			}				
		}
	}
...
// Legacy
	...
	public function set_prop($field, $value, $update_meta = true ){
